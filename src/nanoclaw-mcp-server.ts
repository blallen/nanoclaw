/**
 * Host-runnable Stdio MCP Server for NanoClaw.
 *
 * Twin of container/agent-runner/src/ipc-mcp-stdio.ts, but with an
 * env-configurable IPC directory (NANOCLAW_IPC_DIR) so it can write to the
 * host IPC path (e.g. data/ipc/main/) instead of the container mount
 * (/workspace/ipc). It writes the same IPC JSON files the host watcher
 * (src/ipc.ts) consumes. Context still comes from environment variables set
 * by the runner.
 */
...
export function resolveIpcDir(env: NodeJS.ProcessEnv): string {
  return env.NANOCLAW_IPC_DIR ?? '/workspace/ipc';
}
...
const IPC_DIR = resolveIpcDir(process.env);
const MESSAGES_DIR = path.join(IPC_DIR, 'messages');
const TASKS_DIR = path.join(IPC_DIR, 'tasks');
const IPC_SENT_SENTINEL = path.join(IPC_DIR, 'input', '_sent');

// Context from environment variables (set by the runner)
...
server.tool(
  ...
  async (args) => {
    ...
    // Signal to the runner that send_message was used this turn
...
server.tool(
  ...
  {},
  async () => {
    const snapshotPath = path.join(IPC_DIR, 'mcp_servers.json');
    try {
...
// Guard: only open the stdio transport when executed directly, not when
// imported by tests. Mirrors the direct-run check in src/index.ts.
const isDirectRun =
  process.argv[1] &&
